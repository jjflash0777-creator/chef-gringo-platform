...
const SIGNALS: Signal[] = [
  ...
  { intent: "equipment_troubleshooting", weight: 8, test: (t) => has(t, /\b(not cooling|not heating|won'?t start|stopped working|broken|diagnose|tripping breaker|warm freezer|not holding temp|high[- ]limit|work live|bypass)\b/i) || (has(t, /\brepair\b/i) && !has(t, /\b(compare|versus|vs\.?|replacement|replace)\b/i)) },
  ...
  { intent: "recipe_help", weight: 7, test: (t) => has(t, /\b(hawaiian[- ]style braise|no char grill|without a char grill|oktoberfest dinner|production timeline .* dinner)\b/i) },
  { intent: "ingredient_substitution", weight: 7, test: (t) => has(t, /\b(instead of heavy cream|replace heavy cream|lower saturated fat|out of cornstarch|closest substitute)\b/i) },
  { intent: "food_safety", weight: 7, test: (t) => has(t, /\b(cleaning and sanitizing|cleaning vs sanitizing|difference between cleaning and sanitizing|recalls? .* today|announced today .* recall)\b/i) },
  { intent: "food_cost_labor", weight: 7, test: (t) => has(t, /\b(culinary revenue|department needs more revenue|revenue per occupied unit|under budget overall|food is over budget|extra person tomorrow|running out of one entree|leftovers|activities added \d+ guests)\b/i) },
  ...
  { intent: "ingredient_substitution", weight: 4, test: (t) => has(t, /substitut|\bswap\b|instead of|don['’]t have|do not have|out of|replace .{1,50} with|alternative to|can i use .{1,40} instead/i) && !has(t, /\b(substitute supplier|supplier category|run out of|running out of|leftovers?)\b/i) },
