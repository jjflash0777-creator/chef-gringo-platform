...
export function productCardViewModel(product: ProductRecord) {
  ...
  const imageAuthorized = product.image.licensing === "authorized" || product.image.licensing === "licensed";
  return { ...product, recommendation: product.editorial.badge, attributes: product.editorial.strengths.slice(0, 3), purchase, media: { kind: "primary" as const, url: imageAuthorized ? product.image.referenceUrl : null, sourceUrl: product.image.referenceUrl, sourceLabel: product.image.provenance, rights: imageAuthorized ? "authorized" as const : "reference_only" as const, alt: product.image.alt ?? `${product.name} product image` }, pricePresentation: { label: basis === "unknown" ? "Price context" : /–|-/u.test(product.price.context) ? "Typical observed range" : "Best verified price", value: product.price.context, basis, basisLabel: priceBasisLabel(basis, purchase.href !== null), verifiedAt: product.price.checked, unknownCosts: ["Shipping", "Installation", "Taxes and final delivered cost"] } };
}
