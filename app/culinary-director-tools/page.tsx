...
export default function ToolsPage() {
  return <div className="page-shell">
    <div className="container"><p className="breadcrumbs"><a href="/">Home</a> / Culinary Director Tools</p><p className="eyebrow">For the professional kitchen</p><h1>Tools that earn their spot on the clipboard.</h1><p className="lede">Practical resources for the work behind a reliable senior-living dining program. The scaler is live; everything else is clearly marked in development.</p></div>
    <div className="container resource-grid">{groups.map(([group, ...tools]) => <section className="resource-card" key={group}><h2>{group}</h2>{tools.map((tool) => tool === "Recipe scaler" ? <Link href="/tools/recipe-scaler" key={tool}><strong>{tool}</strong><span className="status active">Ready now</span></Link> : <div key={tool}><strong>{tool}</strong><span className="status">Upcoming</span></div>)}</section>)}</div>
