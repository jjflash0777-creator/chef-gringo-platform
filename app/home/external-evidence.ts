...
export type ExternalEvidenceResult = {
  ...
  document: { id: string; fileName: string; mediaType: EvidenceMediaType; sourceType: ExternalSourceType; validation: SourceValidation; extractedAt: string; contentHash: string };
...
export function extractExternalEvidence(input: ExternalEvidenceInput) {
  ...
  const validation = authorityBySource[input.sourceType];
  ...
  return { validation, facts: facts.filter((item): item is ExtractedExternalFact => item !== null), quote };
}
...
export function ingestExternalEvidence(original: InvestigationCase, input: ExternalEvidenceInput): ExternalEvidenceResult {
  ...
  return { updatedCase, document: { id: documentId, fileName: input.fileName, mediaType: input.mediaType, sourceType: input.sourceType, validation: conflicts.length ? "conflicting_source" : extracted.validation, extractedAt: input.extractedAt, contentHash: hash(input.contentText) }, establishedFacts: extracted.facts, unresolved, conflicts, quote: extracted.quote, stateBefore: original.status, stateAfter: updatedCase.status };
}
