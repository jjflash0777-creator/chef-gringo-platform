...
export const metadata: Metadata = { title: "Chef Gringo Specialized Diets", description: "Evidence-led menu conversion tools, specialized-diet templates, nutrition education, and clinically sourced food guidance for professional kitchens." };
...
const dietTools = [
  ...
  { title: "Ingredient & Interaction Checker", status: "Safety layer", copy: "Flag common medication-food and supplement-food interaction questions and route higher-risk decisions to a pharmacist, physician, or registered dietitian." },
] as const;
...
export default function SpecializedDietsPage() {
  return <main className={styles.page}>
    <section className={styles.hero}><div className={styles.heroGrid}><div><p className={styles.kicker}>CHEF GRINGO · SPECIALIZED DIETS</p><h1>Clinical nutrition translated into food people will actually eat.</h1><p className={styles.lede}>Menu conversion, evidence-led nutrition education, professional templates, and commercial tools for kitchens that need something more useful than a stack of dietary restrictions.</p><div className={styles.actions}><a className={styles.primary} href="#menu-converter">Try the converter</a><a className={styles.secondary} href="#tools">See the product roadmap</a><Link className={styles.secondary} href="/culinary-director-tools">Culinary Director Tools</Link></div></div><aside className={styles.manifesto}><span>THE RULE</span><strong>Evidence first. Food second. Monetization third.</strong><p>Sharp claims require sharp sourcing. Uncertainty stays visible.</p></aside></div></section>
    <section className={styles.intro}><div><p className={styles.eyebrow}>WHY THIS EXISTS</p><h2>Most “special diets” fail in the space between the care plan and the plate.</h2></div><p>Chef Gringo Specialized Diets is built for that gap: take clinical requirements, dietary goals, resident preferences, kitchen constraints, cost, staffing, texture, and service reality — then turn them into practical menus and decision tools.</p></section>
    <MenuConversionStudio />
    <section className={styles.toolSection} id="tools"><div className={styles.sectionHeading}><p className={styles.eyebrow}>PRODUCT ROADMAP</p><h2>Tools that can become products, lead magnets, and recurring operator utilities.</h2></div><div className={styles.toolGrid}>{dietTools.map((tool) => <article key={tool.title} className={styles.toolCard}><span>{tool.status}</span><h3>{tool.title}</h3><p>{tool.copy}</p>{"href" in tool ? <a href={tool.href}><strong>Open prototype →</strong></a> : <small>Queued after conversion validation</small>}</article>)}</div></section>
    <section className={styles.matrixSection}><div className={styles.sectionHeading}><p className={styles.eyebrow}>THE MITOPHAGY PROJECT · REFRAMED FOR CHEF GRINGO</p><h2>A health-sovereignty editorial layer without pretending nutrition is a prescription pad.</h2></div><div className={styles.moduleList}>{modules.map(([number,title,copy]) => <article key={number} className={styles.moduleRow}><span>{number}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></section>
    ...
    <section className={styles.conversionSection}><div><p className={styles.eyebrow}>REVENUE PATH</p><h2>Make the menu tools the front door.</h2><p>Free conversion tools and templates attract culinary directors and operators. Premium template packs, professional workflow tools, qualified product referrals, and partner services monetize the same workflow without turning the site into a supplement catalog.</p></div><div className={styles.funnel}><span>Free menu converter</span><b>→</b><span>Email / saved plan</span><b>→</b><span>Premium templates</span><b>→</b><span>Qualified commercial routes</span></div></section>
