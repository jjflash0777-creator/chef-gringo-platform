...
type InitialIngredient = { name: string; quantity: number | string; unit: string };

export function RecipeScaler({
  initialName = "House Soup",
  originalAmount = 8,
  desiredAmount = 24,
  basisLabel = "servings",
  initialIngredients = [
    { name: "Broth", quantity: 6, unit: "cup" },
    { name: "Diced vegetables", quantity: 1.5, unit: "pound" },
  ],
}: {
  initialName?: string;
  ...
  const [desired, setDesired] = useState(String(desiredAmount));
  const [rows, setRows] = useState<Row[]>(initialIngredients.map((item, index) => ({
    id: index + 1,
    name: item.name,
  ...
  function update(id: number, key: keyof Row, value: string) {
  ...
  function calculate() {
    ...
    rows.forEach((row, index) => {
      ...
      if (!Number.isFinite(Number(row.quantity)) || Number(row.quantity) <= 0) {
        nextErrors[`quantity-${index}`] = "Enter a quantity greater than zero.";
      }
    });
    ...
    setScaled(rows.map((row) => ({
      ...row,
      quantity: formatQuantity(scaleQuantity(row.quantity, original, desired)),
    })));
    trackEvent("recipe_scaler_used", {
  ...
  function print() {
    trackEvent("recipe_scaler_printed", { recipeName: name });
    window.print();
  }
  ...
  return (
    <section className="scaler">
      <div className="scaler-controls">
        ...
        <label>
          Original {basisLabel}
          <input inputMode="decimal" value={original} onChange={(e) => setOriginal(e.target.value)} aria-invalid={!!errors.original} />
          <Error text={errors.original} />
        </label>
        <label>
          Desired {basisLabel}
          <input inputMode="decimal" value={desired} onChange={(e) => setDesired(e.target.value)} aria-invalid={!!errors.desired} />
          <Error text={errors.desired} />
        </label>
      </div>
      <div className="ingredient-editor">
        <div className="ingredient-head">
          <h2>Ingredients</h2>
          <button className="text-button" type="button" onClick={() => setRows([...rows, { id: Date.now(), name: "", quantity: "", unit: "each" }])}>
            + Add ingredient
          </button>
        </div>
        {rows.map((row, index) => (
      ...
      {scaled && (
        <div className="scaled-output" aria-live="polite">
          <div>
            <p className="eyebrow">Scaled production recipe</p>
            <h2>{name || "Untitled recipe"}</h2>
            <p className="formula">
              Scaling factor = desired {basisLabel} ÷ original {basisLabel} = {desired} ÷ {original} = {formatQuantity(Number(desired) / Number(original))}
            </p>
          </div>
